%
% !!!!!!!!!! do some extra work so that each apply is faster !!!!!!!!!!
% !!!!!!!!!! The factorization will only work for matvecs    !!!!!!!!!!
% !!!!!!!!!! with the modified matvec routine                !!!!!!!!!!
%
%
...
function F = rskelf_mv_only(A,x,occ,rank_or_tol,pxyfun,opts)
  start = tic;
...
%       F.factors(n).U = U;
      ...
      % for fast multiply, remember to later combine top level
      nsk = length(sk);
      nrd = length(rd);
      sk_loc = 1:nsk;
      rd_loc = (nsk+1):(nsk+nrd);
      F.factors(n).Adown = [eye(nsk), E; T', L + T'*E]; 
      F.factors(n).Aup = [eye(nsk), T; U*[E', F.factors(n).Adown(rd_loc,rd_loc)']];
  ...
  F.factors(n).Aup = F.factors(n).Adown*F.factors(n).Aup;
  if opts.verb
    fprintf(['-'*ones(1,80) '\n'])
    toc(start)
  end
